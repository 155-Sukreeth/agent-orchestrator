...
async def delete_workflow(workflow_id: int, db: AsyncSession = Depends(get_db), current_org: Organization = Depends(get_current_org)):
    success = await workflow_service.delete_workflow(db, workflow_id, current_org.id)
    if not success:
